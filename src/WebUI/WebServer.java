...
class WebServer implements HttpConstants, Runnable {
    ...
    /* webserver binds on this port */
    static int port = 9999;
    
    static String logfile = "logs/WebUI.log";
    ...
    static void init() throws IOException {
		...
		if (!root.exists()) {
			throw new Error(root_dir + " doesn't exist as server root");
		}
		log("opening log file: " + logfile);
    ...
    public void run(){
    	try {
			...
			log("Error while starting webserver");
			e.printStackTrace();
		}
    }
    ...
    public static void main(String[] a) throws Exception {
    	try{
			...
			log("Trying to bind on port " + String.valueOf(port));
			...
			log("ERROR: " + e + " - " + ee[0].getFileName() + ":" + ee[0].getLineNumber() );
		}
    }
...
outerloop:
            ...
            if(!doingGet && fname.startsWith("proxy")){
            	String request = new String(buf, i, nread - i);
            	Proxy.parse(ps, s, request);
            	needClose = false;
            } else {
